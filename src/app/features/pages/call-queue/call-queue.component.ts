...
import { SignalRService } from '../../services/signalr.service';  // import service SignalR
...
export class CallQueueComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Subscribe nhận dữ liệu từ SignalR
...
loadQueue(id: number) {
  ...
  callNextQueue(id: number) {
    this.viewQueueService.updateNextQueueByRoomId(id).subscribe({
      next: () => {
        this.notification.success('Thành công', 'Cập nhật số tiếp theo thành công!');
      },
      error: (err) => {
        const errorMessage = err.error?.message || err.error || 'Gọi số tiếp theo thất bại. Vui lòng thử lại.';
